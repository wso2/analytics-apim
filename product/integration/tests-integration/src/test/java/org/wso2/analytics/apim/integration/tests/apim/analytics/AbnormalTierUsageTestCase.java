...
public class AbnormalTierUsageTestCase extends APIMAnalyticsBaseTestCase {
	...
	public void testAbnormalTierUsageAlert() throws Exception {
		...
		// test case #1
		boolean testOne = isAlertReceived(0,
				...
				10, 1000);
		Assert.assertTrue(testOne, "Abnormal request alert is not received for application: sampleApplication for api_version: svm:v1.0.0");
		...
		boolean testTwo = isAlertReceived(0,
				...
				10, 1000);
		Assert.assertFalse(testTwo, "Incorrect user alert is received for application: sampleApplication for api_version: tree:v1.0.0");
	...
	private void publishDataset() throws Exception {
		...
		// for a given applicationId, api_version, last 4 days ( hence should not
		// receive an alert)
		...
		for (int day = 0; day < 30; day++) {
			...
			for (int request = 0; request < maxLimit; request++) {
				...
				String[] currentReq = buildRequst(clientType, consumerKey, context, "boost:v1.1.0", "boost",
						...
						hostName, apiPublisher, applicationName, applicationId, userAgent, tier, throttledOut, clientIp,
						applicationOwner);
				eventDto.setAttributeValues(currentReq);
				publishEvent(eventDto);
				Thread.sleep(10);
