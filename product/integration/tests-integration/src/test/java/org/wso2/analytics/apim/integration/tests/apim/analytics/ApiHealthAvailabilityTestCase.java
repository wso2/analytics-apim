...
public class ApiHealthAvailabilityTestCase extends APIMAnalyticsBaseTestCase {
    ...
    @Test(groups = "wso2.analytics.apim", description = "Tests if the simulation data is published", dependsOnMethods = "testResponseTimeTooHighAlert")
    public void test1stResponseCountSimulationDataSent() throws Exception {
        deleteData(-1234, RESPONSE_STREAM_NAME.replace('.', '_'));
        Thread.sleep(5000);
        pubishEventsFromCSV(TEST_RESOURCE_PATH, "response.csv", getStreamId(RESPONSE_STREAM_NAME, RESPONSE_STREAM_VERSION), 500);
        Thread.sleep(55500);
    ...
    public void test2ndResponseCountSimulationDataSent() throws Exception {
        pubishEventsFromCSV(TEST_RESOURCE_PATH, "response2.csv", getStreamId(RESPONSE_STREAM_NAME, RESPONSE_STREAM_VERSION), 500);
        Thread.sleep(53500);
        long requestEventCount = getRecordCount(-1234, RESPONSE_STREAM_NAME.replace('.', '_'));
        boolean eventsPublished = false;
        eventsPublished =(requestEventCount==22);
