...
var Helpers = {
  ...
  getDomainBox: function (props, fullDomain, selectedDomain) {
    ...
    var fullCoordinates = Selection.getDomainCoordinates(props, fullDomain);
    var selectedCoordinates = Selection.getDomainCoordinates(props, selectedDomain);
  ...
  getSelectionMutation: function (point, box, dimension) {
    var x = point.x,
        y = point.y;
    var x1 = box.x1,
        x2 = box.x2,
  ...
  panBox: function (props, point) {
    ...
    var selectedDomain = _defaults({}, props.selectedDomain, domain);
    var fullDomain = _defaults({}, props.fullDomain, domain);

    var _ref = props.x1 ? props : this.getDomainBox(props, fullDomain, selectedDomain),
        x1 = _ref.x1,
        x2 = _ref.x2,
        y1 = _ref.y1,
        y2 = _ref.y2;
  ...
  onMouseDown: function (evt, targetProps) {
    ...
    var dimension = targetProps.dimension,
        ...
        onDomainChange = targetProps.onDomainChange,
        cachedSelectedDomain = targetProps.cachedSelectedDomain,
        domain = targetProps.domain;

    var selectedDomain = _defaults({}, targetProps.selectedDomain, domain);
    var fullDomainBox = targetProps.fullDomainBox || this.getDomainBox(targetProps, domain);
    var currentDomain = _isEqual(selectedDomain, cachedSelectedDomain) ? targetProps.currentDomain || selectedDomain || domain : selectedDomain || domain;
    ...
    if (activeHandles) {
    ...
    } else if (this.withinBounds({ x: x, y: y }, domainBox) && !_isEqual(domain, currentDomain)) {
      ...
      }, this.getSelectionMutation({ x: x, y: y }, domainBox, dimension));

      if (_isFunction(onDomainChange)) {
        onDomainChange(minimumDomain, _defaults({}, mutatedProps, targetProps));
      }
      return [{
        target: "parent",
        mutation: function () {
  ...
  onMouseMove: function (evt, targetProps) {
    ...
    if (isPanning) {
      ...
      if (_isFunction(onDomainChange)) {
        onDomainChange(currentDomain, _defaults({}, mutatedProps, targetProps));
      }
      return [{
        target: "parent",
        mutation: function () {
      ...
      if (_isFunction(onDomainChange)) {
        onDomainChange(_currentDomain, _defaults({}, _mutatedProps, targetProps));
      }
      return [{
        target: "parent",
        mutation: function () {
  ...
  onMouseUp: function (evt, targetProps) {
    ...
    if (x1 === x2 || y1 === y2) {
      ...
      if (_isFunction(onDomainChange)) {
        onDomainChange(domain, _defaults({}, mutatedProps, targetProps));
      }
      return [{
        target: "parent",
        mutation: function () {
...
export default _extends({}, Helpers, {
  ...
  onMouseMove: _throttle(attachId(Helpers.onMouseMove.bind(Helpers)), 16, // eslint-disable-line no-magic-numbers
